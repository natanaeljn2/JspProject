			...
			if (acao != null && !acao.isEmpty() && acao.equalsIgnoreCase("deletar")) {
				...
				if (modelLogin.getFotouser() != null && !modelLogin.getFotouser().isEmpty()) {
					...
					response.getOutputStream()
							.write(Base64.getDecoder().decode(modelLogin.getFotouser().split("\\_")[1]));
				...
				byte[] relatorio = new ReportUtil().geraReltorioPDF(modelLogins, "resl-user-jsp", params,
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (request.getContentType() != null && request.getContentType().toLowerCase().startsWith("multipart/")) {
				...
				Part part = request.getPart("fileFoto"); // Pega o arquivo enviado no form

				if (part != null && part.getSize() > 0) {
					byte[] foto = part.getInputStream().readAllBytes(); // Java moderno
					String tipo = part.getContentType().split("/")[1];

					// Codifica para Base64 (com Java nativo)
					String imagemBase64 = "data:image/" + tipo + ";base64," + Base64.getEncoder().encodeToString(foto);

					modelLogin.setFotouser(imagemBase64);
					modelLogin.setExtensaofotouser(tipo);
				}
			}
